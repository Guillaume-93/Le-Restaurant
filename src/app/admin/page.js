...
import { ArrowLeftStartOnRectangleIcon, Bars3Icon, ArrowUturnLeftIcon, XMarkIcon } from '@heroicons/react/24/outline';
import "@/app/globals.css";
import { useState, useEffect } from 'react';
import { useSession, signIn, signOut } from 'next-auth/react'; // Importation des fonctions d'authentification
import Link from 'next/link';
import MenuSectionForm from '@/components/admin/MenuSectionForm';
import ScrollToTop from '@/components/ui/ScrollToTop';
import { useRouter } from 'next/navigation'; // Utilisation de next/navigation pour naviguer
...
export default function Admin() {
    const { data: session, status } = useSession(); // Vérification de la session utilisateur
    ...
    useEffect(() => {
        if (status === 'authenticated') {
            // Vérifiez si l'utilisateur est autorisé à accéder à la page d'administration
            if (session?.user?.role !== 'admin') {
                toast.error("Vous n'êtes pas autorisé à accéder à cette page.");
                router.push('/unauthorized');
            } else {
                // Charger les données si l'utilisateur est un administrateur
                fetch('/api/menu-data', {
                    credentials: 'include',
                })
                    .then((res) => {
